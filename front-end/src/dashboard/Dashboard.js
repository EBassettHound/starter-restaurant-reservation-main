 ...
 useEffect(()=>{
  ...
  return (
    <main>
      ...
      {reservations.map((reservation) => {return <SingleReservation reservation = {reservation} key = {reservation.reservation_id}/>})}
      </div>
      
      <br/>
      <ErrorAlert error={tablesError} />
      <div className = "row row-cols-1 row-cols-md-3">
        {tables.map((table) => {return <SingleTable table={table} loadDashboard = {loadDashboard} key = {table.table_id}/>})}
      </div>
    </main>
  );
}
